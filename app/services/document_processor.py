    ...
    def load_documents(self) -> List[Document]:
        ...
        if os.path.exists(self.config.CV_PDF_PATH):
            ...
            try:
                ...
                print(f"✓ CV procesado: {len(pdf_docs)} páginas")
            except Exception as e:
                print(f"✗ Error procesando CV: {e}")
        else:
            print(f"✗ Archivo CV no encontrado en: {self.config.CV_PDF_PATH}")
        
        # Load TXT (Experience details)
        if os.path.exists(self.config.EXPERIENCE_TXT_PATH):
            print(f"✓ Archivo Experience encontrado")
            try:
                ...
                print(f"✓ Experience procesado: {len(txt_docs)} documentos")
            except Exception as e:
                print(f"✗ Error procesando Experience: {e}")
        else:
            print(f"✗ Archivo Experience no encontrado en: {self.config.EXPERIENCE_TXT_PATH}")
